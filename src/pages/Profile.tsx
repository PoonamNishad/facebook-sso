...
const Profile = ({ user, pages }) => {
  ...
  useEffect(() => {
    ...
    // if (token) {
    //   axios
    //     .get(`${process.env.REACT_APP_BACKEND_URL}/user/pagesInsights?pageId=${selectedPage}`, { headers: { Authorization: `Bearer ${fbToken}` } })
    //     .then((res) => {
    //       setInsights(res.data.data)
    //     })
    //     .catch(() => setInsights(null));
    // }
            const filteredData = dummyInsights[selectedPage];
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center pt-10">
      ...
      {selectedPage && <div className="mt-6"><PageInsightsCards insights={insights} /></div>}
    </div>
  );
};
